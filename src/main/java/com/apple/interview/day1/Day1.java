...
public abstract class Day1 {
    protected static final int[] directions = new int[]{0, 1, 0, -1, 0};
    ...
    public static void main(String[] args) {
        ...
        int distance = v1.computeDistance(input);

        System.out.println(distance);
        System.out.println(v2.computeDistance(input));
    }

    public int computeDistance(String steps) {

        GridPoint destination = computeDestination(steps.split(", "));

        return Math.abs(destination.x) + Math.abs(destination.y);
    }

    protected abstract GridPoint computeDestination(String[] steps);
